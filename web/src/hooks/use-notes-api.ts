...
export const useNotesApi = () => {
  ...
  const notesApiService = useMemo(
    () => createNotesApiService(authApi),
    [authApi]
  );
  ...
  const loadNotes = useCallback(async () => {
    try {
      ...
      const convertedNotes = data.map((note) => ({
        ...
        webhookData:
          note.metadata?.type && note.metadata.processedAt
            ? {
    ...
    async (newNote: Omit<Note, "id">) => {
      try {
        ...
        const convertedResult = {
          ...
          webhookData:
            result.metadata?.type && result.metadata.processedAt
              ? {
    ...
    async (id: string, updates: Partial<Note>) => {
      try {
        ...
        if (updates.description !== undefined)
          apiData.content = updates.description;
        ...
        setNotes((prev) =>
          prev.map((note) => (note.id === id ? convertedResult : note))
        );
        return convertedResult;
      } catch (err) {
        const errorMessage =
