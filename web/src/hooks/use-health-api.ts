...
export const useHealthApi = () => {
  ...
  const healthApiService = useMemo(
    () => createHealthApiService(authApi),
    [authApi]
  );
  ...
  const loadTodayData = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load today data"
      );
  ...
  const createHealthData = useCallback(
    async (data: HealthDataCreate) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to create health data";
  ...
  const updateHealthData = useCallback(
    async (id: number, data: HealthDataUpdate) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to update health data";
        setError(errorMessage);
        throw new Error(errorMessage);
      }
    },
  ...
  const deleteHealthData = useCallback(
    async (id: number) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to delete health data";
        setError(errorMessage);
        throw new Error(errorMessage);
      }
    },
  ...
  const getByDate = useCallback(
    async (date: string) => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to get data by date"
        );
