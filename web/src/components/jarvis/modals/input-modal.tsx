...
import {
  UrlInput,
  TwitterInput,
} from "~/components/jarvis/modals/input-types/url-input";
...
export const InputModal = React.memo(
  ({
    ...
    const handleSubmit = async () => {
      ...
      switch (type) {
        case "twitter":
          validationResult = validateTwitterInput(
            twitterUsername,
            twitterUrl,
            baseData
          );
        ...
        case "loom":
        case "article":
          validationResult = validateUrlInput(
            inputValue,
            type,
            limitPages,
            baseData
      ...
      if (!validationResult.isValid) {
        alert(
          "Por favor, preencha os campos necessários ou selecione um arquivo."
        );
        return;
      }
    ...
    const renderContent = () => {
      ...
      return (
        <p className="text-gray-400">Tipo de conteúdo não suportado: {type}</p>
      );
    };
    ...
    return (
      <>
        <div className="fixed inset-0 z-[55] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md">
          <div className="relative w-full max-w-lg rounded-3xl border border-white/10 bg-white/[0.05] p-6 shadow-2xl backdrop-blur-md sm:p-8">
            <div className="mb-6 flex items-center justify-between sm:mb-8">
              ...
              <h2 className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-xl font-bold text-white sm:text-2xl">
                {title}
              </h2>
              <Button
                variant="ghost"
            ...
            <Button
              ...
              className="mt-6 w-full text-base sm:mt-8"
              size="lg"
            >
              {isLoading ? (
