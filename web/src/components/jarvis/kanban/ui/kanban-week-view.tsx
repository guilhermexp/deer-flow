...
export default function KanbanWeekView({
  ...
  const [currentMobileColumnIdx, setCurrentMobileColumnIdx] =
    useState<number>(0);
  ...
  useEffect(
    () =>
      saveToLocalStorage(MOBILE_COLUMN_KEY, currentMobileColumnIdx.toString()),
    [currentMobileColumnIdx]
  );
  ...
  const goToPrevCol = () =>
    setCurrentMobileColumnIdx((prev) => Math.max(0, prev - 1));
  const goToNextCol = () =>
    setCurrentMobileColumnIdx((prev) =>
      Math.min(columnsWithTasks.length - 1, prev + 1)
    );
  const handlePanEnd = (
  ...
  const handleDropOnWeekColumn = useCallback(
    (e: React.DragEvent, day: TaskWeekDay) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  return (
    <div className="kanban-week-view-container flex h-full flex-col bg-[#0a0a0a] text-gray-100">
      {isMobile ? (
        <div className="flex flex-1 flex-col overflow-hidden px-2">
          <div className="mb-4 flex items-center justify-between">
            <Button
              ...
              className="border-white/10 bg-white/[0.05] text-gray-400 hover:bg-white/[0.08] hover:text-gray-100 disabled:opacity-50"
            >
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {columnsWithTasks.length > 0 &&
                  columnsWithTasks[currentMobileColumnIdx] && (
                    <KanbanDayColumn
                      column={columnsWithTasks[currentMobileColumnIdx]!}
                      isDragOver={
                        dragOverColumnId ===
                        columnsWithTasks[currentMobileColumnIdx]?.id
                      }
        ...
        <div className="relative flex-1 overflow-hidden px-2">
          <div
            ...
            className="kanban-week-scroll-container flex h-full gap-3 overflow-x-auto pb-2"
          ...
          {showScrollButtons.left && (
            <motion.div
              ...
              className="absolute top-1/2 left-0 z-10 ml-2 -translate-y-1/2"
            >
              <Button
                onClick={scrollToPrev}
                size="icon"
                className="h-10 w-10 rounded-full border border-white/10 bg-white/[0.05] shadow-lg backdrop-blur-sm hover:bg-white/[0.08]"
          ...
          {showScrollButtons.right && (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.8 }}
              className="absolute top-1/2 right-0 z-10 mr-2 -translate-y-1/2"
            >
              <Button
