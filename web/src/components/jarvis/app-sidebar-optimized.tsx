...
const implementedRoutesSet = new Set([
  "/chat",
  "/dashboard",
  "/calendar",
  "/projects",
    ...
    return (
      <Button
        ...
        className={cn(
          ...
          "flex items-center",
          isMobileOpen
            ? "h-12 w-full justify-start px-4" // Estilo para mobile com label
            : "h-9 w-9 justify-center", // Botões balanceados para sidebar
          "hover:bg-white/10",
        ...
        <span
          className={cn(
            ...
            isMobileOpen
              ...
              : "pointer-events-none absolute left-full z-50 ml-3 rounded-md bg-black/80 px-2 py-1 text-xs text-white opacity-0 group-hover:opacity-100 lg:group-hover:opacity-100", // Tooltip original
  ...
  ({
    ...
    return (
      <div className="flex h-full flex-col">
        <div className="border-border/60 flex h-10 items-center justify-center border-b">
          <Brain className="h-4 w-4 text-slate-100 drop-shadow-[0_0_20px_rgba(255,255,255,0.5)] transition-all hover:brightness-125" />
        </div>

        <nav
          className={cn(
            "flex flex-1 flex-col gap-1.5 px-1.5 py-2",
        ...
        >
          {navItems.map((item) => (
            <div
              key={item.href}
              className={cn("group relative", isMobileOpen && "w-full")}
            >
              <SidebarButton
                item={item}
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  "rounded-md bg-black/80 whitespace-nowrap text-white",
                  "opacity-0 transition-opacity duration-200 lg:group-hover:opacity-100",
...
export const AppSidebar = memo(function AppSidebar({
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="supports-backdrop-blur:bg-background/80 bg-background/40 border-border/60 fixed top-0 bottom-0 left-0 z-30 hidden w-12 border-r backdrop-blur-lg lg:flex">
        <SidebarContent
          pathname={pathname}
        ...
        {isMobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="supports-backdrop-blur:bg-background/90 bg-background/60 border-border/60 fixed top-0 bottom-0 left-0 z-50 w-64 border-r backdrop-blur-lg lg:hidden"
            >
              <Button
                variant="ghost"
                size="icon"
                onClick={toggleMobileSidebar}
                className="absolute top-4 right-[-0.5rem] rounded-full text-slate-300 hover:text-white sm:right-4"
              >
                <X className="h-5 w-5" />
              </Button>
              <SidebarContent
