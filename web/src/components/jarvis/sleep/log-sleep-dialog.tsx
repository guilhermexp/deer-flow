...
export default function LogSleepDialog({
  ...
  useEffect(() => {
    if (isOpen) {
      if (currentLog?.status === "logged_custom") {
        ...
        setCustomBedTime(minutesToTimeStr(currentLog.actualBedTime ?? scheduledBedTime))
        setCustomWakeTime(minutesToTimeStr(currentLog.actualWakeTime ?? scheduledWakeTime))
      } else {
  ...
  const scheduledDuration = calculateSleepDuration(scheduledBedTime, scheduledWakeTime)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={resetAndClose}>
      <DialogContent className="sm:max-w-[425px] bg-[#0a0a0a] border-white/10 text-gray-100">
        <DialogHeader>
          <DialogTitle className="text-white">Registrar Sono para {dayName}</DialogTitle>
          <DialogDescription className="text-gray-400">
            Registre como foi seu sono neste dia. Você pode confirmar o horário agendado ou inserir horários diferentes.
          </DialogDescription>
        </DialogHeader>
        <div className="py-4 space-y-4">
          <p className="text-sm text-gray-400">
            Agendado: {formatTime(scheduledBedTime)} - {formatTime(scheduledWakeTime)} ({scheduledDuration?.hours || 0}h{" "}
            {(scheduledDuration?.minutes || 0) > 0 ? `${scheduledDuration?.minutes || 0}m` : ""})
          </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  className="bg-white/[0.05] border-white/10 text-gray-100 focus:border-white/20 placeholder:text-gray-500"
                />
              </div>
              <div>
              ...
              <Button onClick={handleLogCustom} className="w-full bg-blue-500/20 text-blue-400 border border-blue-500/50 hover:bg-blue-500/30">
