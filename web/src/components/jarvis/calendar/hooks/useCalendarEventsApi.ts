...
export const useCalendarEventsApi = () => {
  ...
  const calendarApiService = useMemo(
    () => createCalendarApiService(authApi),
    [authApi]
  );
  ...
  const addEvent = useCallback(
    async (newEventData: NewEventFormData) => {
      try {
    ...
    async (eventId: string, updates: Partial<CalendarEvent>) => {
      try {
        ...
        if (updates.subtitle !== undefined)
          apiUpdates.description = updates.subtitle;
        if (updates.color) apiUpdates.color = updates.color;
        if (updates.category) apiUpdates.category = updates.category;

        const result = await calendarApiService.update(
          parseInt(eventId),
          apiUpdates
        );
        const convertedResult = apiEventToComponentEvent(result);
        setEvents((prev) =>
          prev.map((event) => (event.id === eventId ? convertedResult : event))
        );
        return convertedResult;
      } catch (err) {
        const errorMessage =
