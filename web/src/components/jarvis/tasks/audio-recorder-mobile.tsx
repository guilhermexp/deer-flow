...
const AudioRecorderMobile = React.memo(
  ({ onRecordingComplete, className }: AudioRecorderMobileProps) => {
        ...
        <AnimatePresence mode="wait">
          {!hasRecording ? (
            <motion.section
              ...
              className="flex w-full flex-col items-center space-y-6 py-6"
            >
              <figure className="flex w-full flex-col items-center gap-3">
                <motion.button
                  ...
                  className={cn(
                    ...
                    isRecording
                      ? "bg-primary border-primary/70 shadow-primary/20 shadow-md"
                ...
                >
                  ...
                  >
                    {isRecording ? (
                      <Square className="text-primary-foreground mx-auto h-8 w-8" />
                    ) : (
                      <Mic className="text-primary mx-auto h-8 w-8" />
                    )}
                  </motion.div>
                  {isRecording && (
                    <motion.div
                ...
                <figcaption className="mt-2 w-full space-y-0.5 text-center">
                  <span className="text-foreground block font-mono text-2xl font-semibold tracking-tight">
            ...
            >
              <div className="bg-muted/30 relative h-16 overflow-hidden rounded-md p-3">
                <div className="flex h-full items-center justify-center space-x-0.5">
              ...
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center space-x-3">
                  <motion.button
                    ...
                    className={cn(
                      "bg-accent-red hover:bg-accent-red/80 h-12 w-12 rounded-full",
                      "flex items-center justify-center transition-colors",
                      "focus:ring-accent-red/30 focus:ring-2 focus:outline-none"
                    )}
                    whileTap={{ scale: 0.95 }}
                ...
                <motion.button
                  ...
                  className={cn(
                    "bg-muted hover:bg-destructive/80 h-10 w-10 rounded-full",
                    "flex items-center justify-center transition-colors",
                    "focus:ring-muted/30 group focus:ring-2 focus:outline-none"
                  )}
                  whileTap={{ scale: 0.95 }}
                  aria-label="Delete recording"
                >
                  <Trash2 className="text-muted-foreground group-hover:text-destructive-foreground h-4 w-4" />
                </motion.button>
              </div>
            </motion.div>
          )}
