...
const calculateHealthScore = (data: HealthData): number => {
  const hydrationPercent =
    data.hydration.goal > 0
      ? (data.hydration.current / data.hydration.goal) * 100
      : 0;
  const hydrationScore = Math.min(hydrationPercent, 100);

  const workoutScoreValue =
    data.workout.weeklyGoal > 0
      ? (data.workout.weeklyCompleted / data.workout.weeklyGoal) * 100
      : 0;
