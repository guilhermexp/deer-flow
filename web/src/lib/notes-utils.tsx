...
export const extractRealThumbnail = (contextType: string, originalUrl: string, youtubeId?: string): string => {
  switch (contextType) {
    case 'youtube':
      if (youtubeId) {
...
export const formatWebhookText = (text: string): React.ReactNode => {
  ...
  return lines.map((line, index) => {
    ...
    if (trimmedLine.toLowerCase().includes('informações') || 
      ...
      if (value) {
        return (
          <div key={index} className="mb-2 flex flex-wrap items-start gap-2">
            <span className="text-primary/80 font-medium min-w-fit">
              {label}:
            </span>
            <span className="text-muted-foreground flex-1">
              {value}
