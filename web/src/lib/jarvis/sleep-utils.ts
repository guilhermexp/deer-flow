...
export function minutesToAngle(minutes: number, totalMinutesInDay: number = 24 * 60): number {
  const normalizedMinutes = ((minutes % totalMinutesInDay) + totalMinutesInDay) % totalMinutesInDay
  const angle = (normalizedMinutes / totalMinutesInDay) * 360
  return (angle - 90 + 360) % 360
}

export function angleToMinutes(angle: number, totalMinutesInDay: number = 24 * 60): number {
...
export function formatDuration(totalMinutes: number, shortForm = false): string {
